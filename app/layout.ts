import { Component, List, ListItem, Widget } from "rayous";
import { SVG } from "./svg";
	...
	build(props: any): Widget {
		return new Widget({
			children: [
				props.page,
				new List({
					class: 'absolute left-24 bottom-0 menu menu-horizontal bg-base-200 rounded-box mt-6',
					items: [
					...
					template(item: any, index: number){
						return new ListItem({
							title: new Widget({class: index == 5 ? 'bg-secondary text-base-100' : 'bg-transparent', children: [SVG(item)]})
						})
					}
				})
			]
